				...
				if ( has_post_thumbnail() ) {
				 ...
				 if( !is_single() ) { ?> 
					 ...
					 <div class="article-content-wrap">
							...
							<?php $link = get_post_permalink($post->ID);  ?>
							<?php $isBe = strpos($link, 'https://youtu.be/' );  ?>
							<?php $isTube = strpos($link, 'https://www.youtube.com/'); ?>
							<?php if( $isBe !== false || $isTube !== false ){ ?>
							<?php $auxlinkCode = $isBe !== false ? explode('.be/', $link) : explode('v=', $link); ?>
						    <?php   // chequeo en el link largo de youtube que no tenga parametros extras despues del codigo, si los tiene los elimino, si no los tengo devuelvo el codigo que se encuentra en la segunda posicion del array, la primera es el dominio.
									if($isTube !== false){
					 				$aux = explode('&', $auxlinkCode[1]);
					 				$linkCode = $aux ? $aux[0] : $auxlinkCode[1];
				 					} else {
					 				$linkCode = $auxlinkCode[1];
				 					} 
						 	?>
							
					 		<div class="iframe-embed span_4">
								<iframe style="border-radius: 5px; height: 100%;" src="https://www.youtube.com/embed/<?php echo $linkCode; ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
							</div>
